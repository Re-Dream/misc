...
if CLIENT then
	...
	if(file.Read("scapblocks.txt",false) == nil or file.Read("scapblocks.txt",false) == "")then
		file.Write("scapblocks.txt","dont do stupid shit here thanks xoxo")
	end
	...
	net.Receive("scap_data", function()
		local scapf = file.Read("scapblocks.txt",false)
		local blocks = {}
		if(scapf != nil)then
			blocks = string.Split(scapf,",")
		end
	...
	hook.Add("OnPlayerChat","commandHook",function(ply,text)
		if (string.StartWith(text,"!scap") or string.StartWith(text,".scap") or string.StartWith(text,"/scap")) and (ply == LocalPlayer()) then
			local playername = string.sub(text,7,string.len(text))
			local done = false
			if(string.len(playername) > 0)then
				for _,ply in pairs(player.GetHumans()) do
					if(ply:Nick():lower():find(playername,1,true) == 1 and !done)then
		...
		if (string.StartWith(text,"!scapblock") or string.StartWith(text,".scapblock") or string.StartWith(text,"/scapblock")) and (ply == LocalPlayer()) then
			local playername = string.sub(text,12,string.len(text))
			local done = false
			if(string.len(playername) > 0)then
				for _,ply in pairs(player.GetHumans()) do
					if(ply:Nick():lower():find(playername,1,true) == 1 and !done)then
						chat.AddText(Color(0,100,0),"Blocked ",Color(255,255,255),ply:Nick())
						local sid = ply:SteamID()
						file.Append("scapblocks.txt",","..sid)
						done = true
					end
				end
			end
		end
		if (string.StartWith(text,"!unscapblock") or string.StartWith(text,".unscapblock") or string.StartWith(text,"/unscapblock")) and (ply == LocalPlayer()) then
			local playername = string.sub(text,14,string.len(text))
			local done = false
			if(string.len(playername) > 0)then
				for _,ply in pairs(player.GetHumans()) do
					if(ply:Nick():lower():find(playername,1,true) == 1 and !done)then
						chat.AddText(Color(0,100,0),"Unblocked ",Color(255,255,255),ply:Nick())
						local sid = ply:SteamID()
						local blockstr = file.Read("scapblocks.txt",false)
						if(blockstr != nil or blockstr != "")then
							file.Write("scapblocks.txt",string.Replace(blockstr,","..sid,""))
						end
						done = true
					end
				end
	...
	concommand.Add("scap",function(ply,cmd,ta,args)
		...
		if (args != "")then
			for _,ply in pairs(player.GetHumans()) do
				if(ply:Nick():lower():find(args,1,true)==1 and !done)then
					RunConsoleCommand("cancelselect")
					selectcapt({ply:SteamID()})
					done = true
				end
	...
	concommand.Add("scapblock",function(ply,cmd,ta,args)
		local done = false
		if (args != "")then
			for _,ply in pairs(player.GetHumans()) do
				if(ply:Nick():lower():find(args,1,true)==1 and !done)then
					MsgC(Color(0,100,0),"Blocked ",Color(255,255,255),ply:Nick(),"\n") 
					local sid = ply:SteamID()
					file.Append("scapblocks.txt",","..sid)
					done = true
				end
	...
	concommand.Add("unscapblock",function(ply,cmd,ta,args)
		local done = false
		if (args != "")then
			for _,ply in pairs(player.GetHumans()) do
				if(ply:Nick():lower():find(args,1,true)==1 and !done)then
					MsgC(Color(0,100,0),"Unblocked ",Color(255,255,255),ply:Nick(),"\n")
					local sid = ply:SteamID()
					local blockstr = file.Read("scapblocks.txt",false)
					if(blockstr != nil or blockstr != "")then
						file.Write("scapblocks.txt",string.Replace(blockstr,","..sid,""))
...
--i hope this garbo doesnt softlock you
